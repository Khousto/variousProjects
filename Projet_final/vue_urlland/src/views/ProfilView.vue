...
import Post from "../components/clientPostComponent.vue";
import Comment from "../components/clientCommentComponent.vue";
...
export default {
  ...
  created() {
    ...
    fetch("http://127.0.0.1:8000/api/user/" + localStorage.getItem("id"))
      .then(function (res) {
        if (res.ok) {
          return res.json();
        }
    ...
    fetch(
      "http://127.0.0.1:8000/api/user/" + localStorage.getItem("id") + "/posts"
    )
      .then(function (res) {
        if (res.ok) {
          return res.json();
...
<template>
    ...
    <div class="affichage_user">
      <form>
        <div class="admin_form">
          ...
          <label>Email : </label>
          <input type="email" class="modif_email" v-model="user.email" />
  ...
  <div class="admin_buttons">
    ...
    <button class="back" id="admin_button" @click="goToAdmin()">
